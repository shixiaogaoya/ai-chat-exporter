/**
 * Qwen Chat 对话导出脚本
 * 使用方法: 在 chat.qwen.ai 页面的控制台中运行此脚本
 */
(function() {
    const platformName = "Qwen Chat";
    ...
    if (items.length === 0) {
        ...
        allDivs.forEach(div => {
            ...
            if (!seen.has(text) && text.length > 5) {
                seen.add(text);
                items.push({ role: isUser ? 'User' : 'Qwen', text });
                isUser = !isUser;
            }
        });
    }
    ...
    a.download = `QwenChat_Export_${new Date().toISOString().slice(0,10)}.txt`;
